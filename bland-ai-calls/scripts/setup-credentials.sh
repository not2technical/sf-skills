...
#
# Configures Bland.ai API credentials for the bland-ai-calls skill
# This script is part of the bland-ai-calls skill and provides a one-command setup
#
# Usage:
...
#
# The script will:
#   1. Prompt for your Bland.ai API key (securely)
#   2. Create API_Credentials__c Custom Setting (if needed)
#   3. Store your API key in the org
...
usage() {
    ...
    echo ""
    echo -e "${YELLOW}Get your Bland.ai API key at:${NC}"
    echo -e "  ${CYAN}https://app.bland.ai/settings/api${NC}"
    exit 1
}
...
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${YELLOW}Enter your Bland.ai API key${NC}"
echo -e "${CYAN}Get it from: https://app.bland.ai/settings/api${NC}"
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
SETTING_CHECK=$(sf data query \
    --query "SELECT Id FROM API_Credentials__c WHERE Name = 'BlandAI' LIMIT 1" \
...
if [ -z "$EXISTING_ID" ]; then
    echo -e "${BLUE}► Creating Bland.ai credential record...${NC}"
    sf data record create \
        --sobject API_Credentials__c \
        --values "Name='BlandAI' API_Key__c='$API_KEY'" \
...
CSP_CHECK=$(sf data query \
    --query "SELECT Id FROM CspTrustedSite WHERE DeveloperName = 'BlandAPI' LIMIT 1" \
    --target-org "$ORG_ALIAS" \
    --json 2>&1 || echo '{"status":1}')
...
if echo "$CSP_CHECK" | grep -q "sObject type 'CspTrustedSite' is not supported"; then
    ...
    REMOTE_SITE_CHECK=$(sf data query \
        --query "SELECT Id FROM RemoteSiteSetting WHERE SiteName = 'BlandAPI' LIMIT 1" \
        --target-org "$ORG_ALIAS" \
        --json 2>&1 || echo '{"status":1}')
    ...
    if [ -z "$REMOTE_SITE_ID" ]; then
        ...
        sf project deploy start \
            --source-dir "$SKILL_DIR/templates/BlandAPI.remoteSite-meta.xml" \
            --target-org "$ORG_ALIAS" \
    ...
    if [ -z "$CSP_ID" ]; then
        ...
        sf project deploy start \
            --source-dir "$SKILL_DIR/templates/BlandAPI.cspTrustedSite-meta.xml" \
            --target-org "$ORG_ALIAS" \
            --wait 5 > /dev/null 2>&1 || true
...
echo ""
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✓ Bland.ai integration configured successfully!${NC}"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
echo ""
echo -e "${YELLOW}1. Deploy the Apex classes:${NC}"
echo -e "   ${CYAN}sf project deploy start --source-dir bland-ai-calls/templates/ --target-org $ORG_ALIAS${NC}"
echo ""
echo -e "${YELLOW}2. Test with Anonymous Apex:${NC}"
echo -e "${CYAN}"
cat << 'EOF'
...
echo -e "${NC}"
echo ""
echo -e "${YELLOW}3. Verify endpoint security (already configured):${NC}"
...
echo -e "   - Quick Start: ${CYAN}bland-ai-calls/QUICKSTART.md${NC}"
echo -e "   - Full Guide: ${CYAN}bland-ai-calls/SKILL.md${NC}"
echo -e "   - Examples: ${CYAN}bland-ai-calls/examples/${NC}"
echo ""

echo -e "${GREEN}🎉 Setup complete! Happy calling!${NC}"
echo ""
